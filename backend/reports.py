...
def render_html_report(record: dict[str, Any]) -> str:
    ...
    trade_rows = "".join(
        "<tr><td>{date}</td><td>{symbol}</td><td>{name}</td><td>{side}</td><td>{price}</td><td>{quantity}</td><td>{pnl}</td></tr>".format(
    ...
    ) or '<tr><td colspan="7">本次回测没有成交记录。</td></tr>'
    event_chips = "".join(
...
<main>
  ...
  <section><h2>最近交易记录</h2><table><thead><tr><th>日期</th><th>代码</th><th>名称</th><th>方向</th><th>价格</th><th>数量</th><th>收益</th></tr></thead><tbody>{trade_rows}</tbody></table></section>
